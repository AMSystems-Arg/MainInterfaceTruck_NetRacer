...
//Dehabilitado ReSharper InconsistentNaming
...
namespace ar.com.amsystems.Telemetry.Server.Helpers
{
    static class Uac
    {
        ...
        public static bool IsProcessElevated()
        {
            ...
            try
            {
                // Abra el token de acceso del proceso actual con TOKEN_QUERY.
                ...
                //Asigne un buffer para la información de eleda..
                ...
                //Recupera información de elevación de token.
                if (!NativeMethods.GetTokenInformation(hToken,
                    TOKEN_INFORMATION_CLASS.TokenElevation, pTokenElevation,
                    cbTokenElevation, out cbTokenElevation))
                {
                     // Cuando el proceso se ejecuta en sistemas S.O antes de Windows
                     // Vista, GetTokenInformation devuelve falso con el código de error
                     // ERROR_INVALID_PARAMETER porque TokenElevation no es compatible
                     // en esos sistemas operativos.
                    throw new Win32Exception();
                }

                // Principal TOKEN_ELEVATION estructura de nativo a objeto .NET.
                TOKEN_ELEVATION elevation = (TOKEN_ELEVATION)Marshal.PtrToStructure(
                    pTokenElevation, typeof(TOKEN_ELEVATION));

                // TOKEN_ELEVATION.TokenIsElevated es un valor distinto de cero si el token
                // tiene privilegios elevados; de lo contrario, un valor cero.
                fIsElevated = (elevation.TokenIsElevated != 0);
            }
            finally
            {
                //  Limpieza centralizada para todos los recursos asignados.
                hToken?.Close();
                if (pTokenElevation != IntPtr.Zero)
    ...
    /// <summary>
     /// El tipo de enumeración TOKEN_INFORMATION_CLASS contiene valores que
     /// especifica el tipo de información asignada o recuperada de un token de acceso.
   /// </summary>
    ...
    /// <summary>
    /// El tipo de enumeración WELL_KNOWN_SID_TYPE es una lista de los más utilizados
    /// identificadores de seguridad (SID). La app pueden pasar estos valores al
    /// función CreateWellKnownSid para crear un SID desde esta lista.
    /// </summary>
